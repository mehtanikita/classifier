$(document).ready(function()
{
	$("#rate_submit").click(function()
	{
		...
		{
			var p = $("#product_no").val();
			$.ajax
			({
				type: "POST",
				url: "product/give_rating",
				data: {rating: rating, product: p},
				success: function(data)
				{
					$("#avg_rating").text(data);
					$("#rate_submit").addClass('disabled');
					$("#rate_submit").fadeOut("slow");
				}
			});
		}
	});

	$('#main_img').wrap('<span style="display:inline-block"></span>').css('display', 'block').parent().
	zoom
	({
		on: 'grab',
		magnify: 3
	...
	$(document).on("click",".img_secondary",function()
	{
		$('#main_img').wrap('<span style="display:inline-block"></span>').css('display', 'block').parent().trigger('zoom.destroy');
	...
	$(document).on("click",".add_to_kart",function()
    {
        ...
        var anim1 = $(this).parents("#product_details");
        var anim = anim1.find("#main_img").clone();
    ...
    $(document).on("click",".remove_from_wishlist",function()
    {
        ...
        ({
            ...
            success: function(data)
            {
                if(data=="true")
                {
                    caller.removeClass("btn-warning remove_from_wishlist");
                    caller.addClass("btn-info add_to_wishlist");
                    caller.find(".btn_text").text(" Add to Wishlist");
                }
                else
                    alert("There was some problem removing the item! Please try again.");
            }
